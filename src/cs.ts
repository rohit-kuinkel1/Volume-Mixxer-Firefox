...
(() => {
  ...
  function connectOutput(element: MediaElement): void {
    if (!tc.vars.gainNode) {
      return;
    }
    element.volume = 1;
  ...
  function init(doc: Document): void {
    ...
    doc.querySelectorAll<MediaElement>("audio, video").forEach((element) => {
      element.volume = 1;
      connectOutput(element);
    });

    const arriveDocument = doc as DocumentWithArrive;
  ...
  function initWhenReady(doc: Document): void {
